...
export default function Signup() {
  ...
  const isSignUpDisabled = !(email && password && confirmPassword && isValidEmail);
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSignUpSubmit}>
              ...
              <TextField label="Email" variant="outlined" fullWidth margin="normal" value={email} onChange={handleEmailChange} error={!isValidEmail && email !== ''} helperText={!isValidEmail && email !== '' ? 'Please enter a valid email address.' : ''} />
              <TextField label="Password" variant="outlined" type={showPassword ? 'text' : 'password'} fullWidth margin="normal" value={password} onChange={handlePasswordChange} InputProps={{ endAdornment: (<InputAdornment position="end"><IconButton onClick={handleShowPasswordClick} edge="end">{showPassword ? <VisibilityOff /> : <Visibility />}</IconButton></InputAdornment>) }} />
              <TextField label="Confirm Password" variant="outlined" type={showConfirmPassword ? 'text' : 'password'} fullWidth margin="normal" value={confirmPassword} onChange={handleConfirmPasswordChange} InputProps={{ endAdornment: (<InputAdornment position="end"><IconButton onClick={handleShowConfirmPasswordClick} edge="end">{showConfirmPassword ? <VisibilityOff /> : <Visibility />}</IconButton></InputAdornment>) }} />
              <AnimateButton><Button variant="contained" color="secondary" fullWidth sx={{ mt: 3, py: 1.5 }} component="label" startIcon={<CloudUploadIcon />}>Upload Avatar<input type="file" hidden onChange={handleAvatarChange} /></Button></AnimateButton>
              {avatarPreview && <Avatar src={avatarPreview} sx={{ width: 80, height: 80, mt: 2, ml: 'auto', mr: 'auto' }} />}
              <AnimateButton><Button variant="contained" color="secondary" fullWidth sx={{ mt: 3, py: 1.5 }} type="submit">Sign Up</Button></AnimateButton>
              <Divider sx={{ flexGrow: 1, mt: 2, mb: 2 }} orientation="horizontal" />
              <Typography variant="body1" sx={{ mt: 2 }}>Already have an account?{' '}<Link to="/signin" style={{ textDecoration: 'none', color: theme.palette.secondary.main }}>Sign In</Link></Typography>
            </form>
          </Box>
        </Box>
      )}
