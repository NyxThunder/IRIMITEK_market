...
function Dashboard() {
  ...
  const { orders, orderLoading, error: ordersError } = useSelector((state) => state.allOrders);
  ...
  const data = products
    ? products.map((item) => [item._id, item.name, item.Stock, item.price])
    : [];
                ...
                <Card sx={{ boxShadow: 3, borderRadius: 2 }}>
                  <HighchartsReact
                    ...
                    options={{
                      ...
                      series: [{ name: 'Revenue', data: [32, 45, 56, 60, 72, metrics.TotalAmount] }]
                    }}
                  />
                </Card>
              </Grid>
