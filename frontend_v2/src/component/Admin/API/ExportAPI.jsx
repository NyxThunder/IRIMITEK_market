...
function ExportAPI() {
  ...
  const {
    ...
    exported,
    message
  } = useSelector((state) => state.exportApi) || {};
  ...
  const validateFields = () => {
    ...
    if (!inventorySize || inventorySize <= 0)
      newErrors.inventorySize = 'Inventory Size must be greater than zero.';
    setErrors(newErrors);
    ...
    { name: 'inventorySize', label: 'Stock' },
    {
      ...
      options: {
        customBodyRender: (value, tableMeta) => (
          <IconButton
            color="secondary"
            onClick={() => dispatch(deleteOffer(offers[tableMeta.rowIndex].productId))}
          >
            <DeleteIcon />
          </IconButton>
        )
      ...
      options: {
        customBodyRender: (value, tableMeta) => (
          <Button
            variant="contained"
            color="primary"
            onClick={() => handleOpenModal(products[tableMeta.rowIndex])}
          >
            Export
          </Button>
        )
            ...
            <Grid item xs={12} sm={12} md={9} lg={9} xl={9}>
              ...
              <Grid container spacing={2} sx={{ mt: 1 }}>
                ...
                <Grid item xs={12}>
                  <Card sx={{ p: 3, boxShadow: 3, borderRadius: 2 }}>
                    <MUIDataTable
                      title={'All Products For DropShipping'}
                      data={products}
